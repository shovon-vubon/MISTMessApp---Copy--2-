...
export default function DeptHeadRecords() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.search) {
      ...
      res = res.filter(r => r.studentName?.toLowerCase().includes(s) || r.serviceNumber?.toLowerCase().includes(s) || r.dept?.toLowerCase().includes(s));
    }
  ...
  return (
    ...
    >
      <Text style={s.heading}>Request Records</Text>
      <Text style={s.sub}>Dept: {profile?.dept || 'ALL'} · {filtered.length} of {all.length}</Text>
        ...
        <View key={r.id} style={s.card}>
          <View style={s.cardHeader}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={s.meta}>{r.serviceNumber} · {r.rank} · <Text style={s.dept}>{r.dept}</Text> · {fmtDate(r.date)}</Text>
            </View>
          ...
          {r.approvedByName && <Text style={s.approvedBy}>Handled by: {r.approvedByName}</Text>}
...
const s = StyleSheet.create({
  ...
  dept:       { color: COLORS.gold, fontWeight: '700' },
  cause:      { color: COLORS.text2, fontSize: 12, marginBottom: 10 },
  times:      { flexDirection: 'row', gap: 12, borderTopWidth: 1, borderTopColor: COLORS.border, paddingTop: 10 },
  timeItem:   { flex: 1 },
  ...
  approvedBy: { color: COLORS.text3, fontSize: 11, marginTop: 8 },
  remarks:    { color: COLORS.amber, fontSize: 11, marginTop: 4 },
});
